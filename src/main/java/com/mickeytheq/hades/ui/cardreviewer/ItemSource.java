...
// java.util.ListIterator has behaviour where next() followed by previous() returns the same element twice
// which is not ideal when trying to build, for example, a dialog that previews a list of card images
//
// by default this iterator starts 'just before' the first element so the first operation should be next() to
